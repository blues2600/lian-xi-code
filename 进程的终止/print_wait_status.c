...
printWaitStatus(const char *msg, int status)
{
    ...
    } else if (WIFSIGNALED(status)) { //信号杀掉子进程
        printf("child killed by signal %d (%s)",
                WTERMSIG(status), strsignal(WTERMSIG(status)));//WTERMSIG=信号编号
#ifdef WCOREDUMP        /* Not in SUSv3, may be absent on some systems */
        if (WCOREDUMP(status)) //进程产生内核转储文件
            printf(" (core dumped)");
#endif
        printf("\n");

    } else if (WIFSTOPPED(status)) { //进程因信号暂停
        printf("child stopped by signal %d (%s)\n",
                WSTOPSIG(status), strsignal(WSTOPSIG(status)));// WSTOPSIG = 停止的信号编号

#ifdef WIFCONTINUED     /* SUSv3 has this, but older Linux versions and
                           some other UNIX implementations don't */
    } else if (WIFCONTINUED(status)) { //收到 SIGCONT 恢复执行
