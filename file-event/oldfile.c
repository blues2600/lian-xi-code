             ...
             struct FTW *ftwbuf)
{
    ...
    printf("[+] %s already added to the monitoring list.\n", fpath);
...
void PrintInotifyEventData(struct inotify_event *p)
{
...
// 监控事件处理函数 - 文件删除处理
int EventProcessDel(struct inotify_event *event)
{
    // 更新路径-描述符表，将当前条目从链表删除
    DeleteElement(event->wd);

    // 移除监控范围
    int retval = inotify_rm_watch(inotify_fd, event->wd);
    if (retval = -1) {
...
// 监控事件处理函数 - 目录删除处理
int EventProcessDelDir(struct inotify_event *event)
{
    PathWatchTable *element = NULL;
    struct inotify_event *event = NULL;
    char dirname[NAME_MAX] = {'\0'};

    element = TableQuery(event->wd);  //获得路径-描述符表的条目指针
    GetNameFromPath(element->path, dirname);  //取出目录名

    // 搜索路径-描述符表，所有归属于dirname目录下的文件都移除监控范围，并删除表条目
    DeleteElement(event->wd);  //先把目录的链表条目删除
                               //
                               //
                               //
                               //到这里
...
int main(int argc, char *argv[])
{
    ...
    for (;;) {
        ...
        if (numRead != 0) {                                //若读取ok
            DisplayEventMsg(msg, numRead);  //输出监控事件消息
            EventProcess                    //根据消息内容处理事件
            //处理的事件有几种
            //1.目录添加 - 文件加入监控范围,watchadd()，该目录有新文件添加，则自动执行2
            //2.文件添加 - 文件加入监控范围,watchadd()
            //3.目录删除 - 扫描路径描述符表，所有该目录下的文件移除监控范围移除路径描述符表
            //4.文件删除 - 移出监控范围移除路径描述符表，新函数
            //6.目录和文件改名 - 路径描述符表的路径要修改
        }
    }
    // 要记得free掉malloc的堆内存
    return 0;
